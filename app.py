...
with st.expander("1️⃣ 왜 필요한가요?", expanded=True):
    ...
    **입학 / 취업 / 승진에 필수!**
    ...
    - 경찰청 등 승진에서 *‘결정적 가산점’* 획득 가능
    """)

with st.expander("2️⃣ 응시 안내"):
    st.subheader("응시 대상")
    st.markdown("""
    - 대한민국 국적을 가진 국민이면 누구나 응시 가능  
    - 외국인의 경우 외국인등록증, 국내거소등록증, 영주증 중 하나 소지 시 응시 가능
    """)

    st.subheader("응시 지역")
    st.markdown("""
    서울, 인천, 수원, 고양, 부산, 울산, 창원, 대구, 광주, 전주, 대전, 청주, 춘천, 강릉, 제주 등  
    **전국 15개 권역**에서 실시하며, 접수 시 고사장 선택 가능
    """)

    st.subheader("응시 방법 및 응시료")
    st.markdown("""
    - KBS한국어능력시험 홈페이지([www.klt.or.kr](https://www.klt.or.kr))에서 온라인/모바일 접수만 가능  
    - 응시료: **33,000원**  
    - 자격증 발급 수수료: **5,000원 (등기 우편)**  
    """)

with st.expander("3️⃣ 시험 구성 및 영역"):
    st.markdown("""
    **출제 방식:** 객관식 5지 선다형, 총 100문항  

    | 영역 | 문항 수 | 시험 시간 |
    |:------|:------:|:------:|
    | 듣기·말하기 | 15문항 | 10:00~10:25 (25분) |
    | 어휘 | 15문항 |  |
    | 어법 | 15문항 |  |
    | 쓰기 | 5문항 | 10:25~12:00 (95분) |
    | 창안 | 10문항 |  |
    | 읽기 | 30문항 |  |
    | 국어문화 | 10문항 |  |
    ...
    **출제 영역**
    ...
    - 이해 능력 (듣기, 읽기)
    - 표현 능력 (쓰기, 말하기)
    - 창의적 언어 사용 능력 / 국어 관련 교양 지식
    """)

with st.expander("4️⃣ 영역별 출제 경향"):
    st.subheader("듣기·말하기 영역")
    st.markdown("""
    - 그림 해설·장면·라디오 듣고 내용 파악  
    - 고전·우화·시 청취 후 관련 내용 추론  
    - 대화·발표 듣고 말하기 방식 추론
    """)
...
st.divider()
st.info("💡 다음 단계로 ‘규정 학습’, ‘어휘 플래시카드’, ‘다의어 학습’을 진행해 보세요!")
